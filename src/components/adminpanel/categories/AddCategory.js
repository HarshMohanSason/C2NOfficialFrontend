...
function AddCategory() {
	...
	const createCategoryFormData = () => {
		...
		formData.append(
			"how_to_measure_image",
			categoryData.how_to_measure_image,
		);
	...
	const uploadCategory = async (event) => {
		...
		try {
			...
			const response = await fetch(
				process.env.REACT_APP_SUBMIT_CATEGORY_DATA,
				{
					method: "POST",
					credentials: "include",
					body: categoryData,
		...
		<section className="add-category-page">
			...
			<form method="post" onSubmit={uploadCategory}>
				<section className="add-category-section">
					...
					<section className="chart-category-input-table">
						...
						<div className="column-buttons">
							<input
								type="button"
								value="Add Column"
								className="size-chart-plus-button"
								onClick={addColumn}
							/>
							<input
								type="button"
								value="Remove Column"
								className="size-chart-minus-button"
					...
					<button type="submit" class="upload-category-button">
						Upload
					</button>
				</section>
			</form>
